...
document.getElementById('themeSelect').addEventListener('change', () => {
  ...
  if (selectedOption === 'default') {
    ...
    location.reload();
  } else if (selectedOption === 'crystalBeast') {
    //宝玉獣
    sessionStorage.clear();
    ...
    createCardElement('card51', 'https://pbs.twimg.com/media/Fz_fXQRacAATW_L?format=jpg&name=small'); //天子の指輪
    ...
    sessionStorage.setItem('selectedBackgroundUrl',
      optionalBackgroundUrl);
    location.reload();
  } else if (selectedOption === 'abyssActor') {
    //魔界劇団
    sessionStorage.clear();
    ...
    sessionStorage.setItem('selectedBackgroundUrl',
      optionalBackgroundUrl);
    location.reload();
  } else if (selectedOption === 'mekkKnight') {
    //ジャックナイツ
    sessionStorage.clear();
    ...
    sessionStorage.setItem('selectedBackgroundUrl',
      optionalBackgroundUrl);
    location.reload();
  } else if (selectedOption === 'magician') {
    //魔術師
    sessionStorage.clear();
    ...
    sessionStorage.setItem('selectedProtectorUrl',
      optionalProtectorUrl);
    const optionalBackgroundUrl = 'https://64.media.tumblr.com/219e357778acffba29294a19d6152a4a/tumblr_paf80mLEKA1rie4kjo1_r1_500.png';
    sessionStorage.setItem('selectedBackgroundUrl',
      optionalBackgroundUrl);
    location.reload();
  }
...
undoButton.addEventListener('click', () => {
  ...
  if (logs[logs.length - 1].actionType === 'moveCard' || logs[logs.length - 1].actionType === 'moveAndZoomCard' || logs[logs.length - 1].actionType === 'moveAndGlowCard') {
    ...
    if (filteredLogs.length === 1) {
      ...
      //const undoanimationType = filteredLogs[filteredLogs.length - 1].animationType;
  ...
  () => {
    ...
    logs.forEach((log,
      ...
      setTimeout(() => {
        ...
        index * 1200); // 秒ごとにアニメーションを再生する（適宜調整してください）
    })
  });