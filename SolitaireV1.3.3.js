...
document.addEventListener('DOMContentLoaded', () => {
  for (let i = 0; i < sessionStorage.length; i++) {
    ...
    if (cardData) {
      ...
      const existingElement = cardBoard.querySelector(`[data-type='${cardObject.type}']`);
      if (existingElement) {
        //対象カードの種類が既にcardBoardに存在する場合
        //alert(`カードID ${imgElement.id} 対象のカードの種類と同じものがcardBoardに存在します。前にinsertします。`);
        cardBoard.insertBefore(imgElement, existingElement);
      } else {
        //対象のカードの種類が存在しない場合
        ...
        if (nextTypeElements.length > 0) {
          ...
          //alert(`カードID ${imgElement.id} 対象のカードの種類と同じものはcardBoardに存在しません。次の種類のカードがあったため、一番若いものの前に挿入します`);
          const nextTypeElement = nextTypeElements.reduce((minElement, element) => typeOrder[element.dataset.type] < typeOrder[minElement.dataset.type] ? element : minElement, nextTypeElements[0]);
          cardBoard.insertBefore(imgElement, nextTypeElement);
        } else {
          //次の種類のカードがどこにもなければ、普通に一番最後に追加する
          //alert(`カードID ${imgElement.id} 対象のカードの種類と同じものはcardBoardに存在しません。さらに次の種類のカードもないので一番末尾にいれます`);
...
themeSelect.addEventListener('change', () => {
  ...
  const createCardElement = (id, imageUrl, type) => {
    ...
    const existingElement = cardBoard.querySelector(`[data-type='${type}']`);
    if (existingElement) {
      //対象カードの種類が既にcardBoardに存在する場合
      //alert(`カードID ${imgElement.id} 対象のカードの種類と同じものがcardBoardに存在します。前にinsertします。`);
      cardBoard.insertBefore(imgElement, existingElement);
  ...
  case 'default':
    {
      ...
      createCardElement('card1', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR6oP6EjWOK6J1vRjzHexMYTsdHEctLiNjsfA&usqp=CAU', 'tr');
      createCardElement('card2', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTAccTgRdl3hQ6t7QBR1KsQMoBGghxvtCjYvA&usqp=CAU', 'xy');
      createCardElement('card3', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRsnOFPVSjsskqFWp0OPjWYCDFxBhLs8h1ycg&usqp=CAU', 'sp');
      createCardElement('card4', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS1-KS0aT7evkr2Aa2_02B48WlQx4TCU74sPw&usqp=CAU', 'ef');
  ...
  case 'infernobleIgKnight':    
    {
      ...
      createCardElement('card39', 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR41aPXM2-6WGXeoJqGZbk8sYd0phxljVloBQ&usqp=CAU', 'li');
